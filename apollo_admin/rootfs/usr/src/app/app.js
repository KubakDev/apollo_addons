...
async function setupSignalRConnection(token, responseTopic, correlationData) {
  try {
    ...
    if (signalr.state === "Connected") {
      ...
      logger.warn("Attempting to reconnect with a new token due to token expiration...");
      const msg = { success: true, result: "ready" };
      sendMqttMessage(config.apolloAdminReady, msg);
    }
  } catch (error) {
  ...
  signalr.onclose(async (error) => {
    ...
    if (isTokenExpiredError(error)) {
      ...
      logger.warn("SignalR connection closed due to an error. Attempting to reconnect...");
      const msg = { success: true, result: "ready" };
      sendMqttMessage(config.apolloAdminReady, msg);
    }
  });
  ...
  signalr.on("Request", async (data) => {
    ...
    if (data.hasResult) {
      ...
      try {
        ...
        logger.error("Error sending MQTT message for 'Request' from SignalR", error);
        return {result:null,error: { code:"0101", message: error.message },success:false}
      }
    }
  });
}
